...
const Search = ({className}: Props) => {
  ...
  return (
    <div className={`${className ?? ''} flex flex-col items-center justify-start absolute inset-x-12 inset-y-12 p-4 gap-4 bg-white dark:bg-gray-800 rounded-2xl shadow-lg text-black dark:text-white text-base font-medium`}>
      ...
      <div className="flex flex-col items-center justify-start w-full h-full overflow-y-scroll gap-2">
        {items.map((item, i) => {
          if(item.type === 'video') {
        ...
        {(currSearch) && <button disabled={loadState === "Loading..."} className="px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 active:bg-gray-400 dark:active:bg-gray-500 transition-colors focus:outline-none" onClick={nextPage}>
          {loadState}
        </button>}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-row items-center justify-start w-full p-2">
      ...
      <div className="flex flex-col items-center justify-start w-full gap-4 p-4 truncate rounded-md" >
        <h1 className="w-full text-xl text-center text-black truncate dark:text-white font-semibold">{title}</h1>
        <span className="w-full gap-4 flex flex-row flex-wrap justify-center items-center">
...
const SearchPlaylist = ({title, url, owner, length, firstVideo }: ytsr.Playlist ) => {
  return (
    <div className="flex flex-row items-center justify-start w-full p-2">
      ...
      <div className="flex flex-col items-center justify-start w-full gap-4 p-4 truncate rounded-md" >
        <h1 className="w-full text-xl text-center text-black truncate dark:text-white font-semibold"><span className="font-bold">[Playlist]</span> {title}</h1>
        <span className="w-full gap-4 flex flex-row flex-wrap justify-center items-center">
