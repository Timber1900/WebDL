...
const Main = () => {
  ...
  return(
    <div ref={scrollRef} onScroll={fixManxRenderable} className={`flex ${curQueue.filter(val => val.show).length > 4 ? 'flex-row flex-wrap items-start justify-center' : 'flex-col items-center justify-start'}  overflow-y-scroll bg-gray-200 dark:bg-gray-700`}>
      {
      ...
      <>
        ...
        <button  className="px-4 py-2 mb-2 transition-colors bg-gray-300 rounded-lg mx-full dark:bg-gray-800 hover:bg-gray-400 dark:hover:bg-gray-800 active:bg-gray-400 dark:active:bg-gray-900 focus:outline-none" onClick={() => setMaxRenderable(maxRenderable + 20)}>
        Load More
        </button>
      </>}
    </div>
