      ...
      .catch((err) => {
        ...
        youtubeDlWrap
          ...
          .then((info) => {
            ...
            const thumbArray = info.thumbnails;
            const thumb = thumbArray ? thumbArray[thumbArray.length - 1].url : defThumb;
            addDiv(url, thumb, info.title, sorted_map, info, 0, false).then((val) => {
              const parent = document.getElementById('playlistSelect');
              parent.appendChild(val);
            });
          })
...
function addDiv(url, thumbnail, title, formats, info, rank, yt) {
  if (!queued_videos.has(url)) {
    ...
    div.children[2].children[1].children[1].children[1].children[0].children[0].setAttribute(
      'onclick',
      'openTrimPopup(this.parentNode.parentNode.parentNode.children[0])',
    );
    ...
    div.children[2].children[1].children[1].children[1].children[0].children[1].setAttribute(
      'onclick',
      `addClip(this, ${hours}, ${min}, ${sec}, ${fullsec})`,
    );
...
const addSearchToQueue = function (e) {
  const id = e.parentNode.parentNode.children[1].children[0].children[0].src.replace(
